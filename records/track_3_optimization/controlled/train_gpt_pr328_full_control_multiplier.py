...
EXPERIMENT_NAME = "pr328-full-control-multiplier"
EXPERIMENT_INTUITION = "PR328 Track 3 stack with optional Muon-only total-feedback multiplier control."
MU = 0.95
MUON_LR = 0.0375
TARGET_UW = 0.3825
...
def set_hparams(step):
    ...
    for opt in optimizers[0].inner_optimizer:
        for group in opt.param_groups:
            ...
            group["lr"] = native_lr
        if CONTROL_ENABLED and opt is optimizer2:
            for group in opt.param_groups:
...
for step in range(train_steps + 1):
    ...
    for i in range(len(inputs) // mbs):
        ...
        if probe_this_step:
            loss_before_probe_sum = (
                loss.detach().float()
                if loss_before_probe_sum is None
                else loss_before_probe_sum + loss.detach().float()
            )
        loss.backward()
    for name, p in model.named_parameters():
    ...
    if probe_this_step:
        ...
        loss_after_probe = torch.zeros((), dtype=torch.float32, device=device)
        with torch.no_grad():
            for i in range(len(inputs) // mbs):
                loss_after_probe += model(
                    inputs[i*mbs:(i+1)*mbs], targets[i*mbs:(i+1)*mbs]
                )
        dist.all_reduce(loss_before_probe, op=dist.ReduceOp.SUM)
        dist.all_reduce(loss_after_probe, op=dist.ReduceOp.SUM)
        control_stats = controller.observe(
            step=step,
            loss_before=loss_before_probe.item(),
            loss_after=loss_after_probe.item(),
